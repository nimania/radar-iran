...
const hour=Math.floor(Date.now()/3600000);const DISCOVERY_LIMIT=Math.max(20,Math.min(30,Number(process.env.DISCOVERY_LIMIT||30)));
...
for(const c of selected){try{const result=await callTool("search_digikala",{query:c.query,limit:DISCOVERY_LIMIT});raw.categories[c.key]={label:c.label,query:c.query,result};normalized.push(...extractProducts(result,{category_key:c.key,category_label:c.label,query:c.query,observed_at:now}))}catch(e){raw.categories[c.key]={label:c.label,error:String(e)}} await new Promise(r=>setTimeout(r,900))}
...
function supplierOf(x){const p=x?.product||x?.data?.product||x?.data||x;const seller=p?.default_variant?.seller||p?.seller||{};return {name:seller?.title||seller?.name||null,url:seller?.url||seller?.page_url||null,website:seller?.website||null,instagram:seller?.instagram||null,telegram:seller?.telegram||null}}
function brandOf(x){const p=x?.product||x?.data?.product||x?.data||x;return p?.brand?.title_fa||p?.brand?.title||p?.brand||null}
const ENRICH_LIMIT=Math.max(0,Number(process.env.ENRICH_LIMIT||40));let enriched=0;
for(const p of fresh){const old=prevMap.get(String(p.source_id));if(!p.image&&old?.image)p.image=old.image;if(!p.brand&&old?.brand)p.brand=old.brand;if(!p.supplier&&old?.supplier)p.supplier=old.supplier;if(enriched>=ENRICH_LIMIT||!p.source_id||(p.image&&p.brand))continue;try{const detail=unwrapDetail(await callTool("product_details",{id:Number(p.source_id)}));p.image=p.image||imageOf(detail);p.brand=p.brand||brandOf(detail);const sup=supplierOf(detail);p.supplier={...(old?.supplier||{}),...Object.fromEntries(Object.entries(sup).filter(([,v])=>v))};enriched++;await new Promise(r=>setTimeout(r,900))}catch(e){raw.categories._enrichment??={};raw.categories._enrichment[p.source_id]=String(e)}}
