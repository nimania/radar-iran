...
for(const p of fresh){const old=prevMap.get(String(p.source_id));if(!p.image&&old?.image)p.image=old.image;if(!p.brand&&old?.brand)p.brand=old.brand;if(enriched>=ENRICH_LIMIT||!p.source_id||(p.image&&p.brand))continue;try{const detail=unwrapDetail(await callTool("product_details",{product_id:Number(p.source_id)}));p.image=p.image||imageOf(detail);p.brand=p.brand||brandOf(detail);enriched++;await new Promise(r=>setTimeout(r,900))}catch(e){raw.categories._enrichment??={};raw.categories._enrichment[p.source_id]=String(e)}}
